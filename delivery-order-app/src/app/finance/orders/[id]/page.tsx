...
export default async function FinanceOrderDetailPage({ params }: PageProps) {
  ...
  const { data: order } = await supabase
    ...
    .select(`
      ...
      delivery_order_flags(id, reason, notes, created_at)
    `)
    .eq("id", id)
    .single()
  ...
  const flags    = (order.delivery_order_flags ?? []) as Array<{
    id: string; reason: string; notes: string | null; created_at: string
  }>
    ...
    <div className="flex flex-col pb-4">
      ...
      {order.photo_url ? (
        <a href={order.photo_url} target="_blank" rel="noopener noreferrer"
          ...
          <img src={order.photo_url} alt={`D.O. ${order.do_number}`} className="w-full object-contain max-h-64" />
        ...
        <div className="mx-4 mt-4 rounded-2xl border border-red-100 bg-red-50 px-4 py-4">
          ...
          {flags.map((flag) => (
            <div key={flag.id}>
              ...
              {flag.notes && <p className="text-xs text-red-600 mt-1">{flag.notes}</p>}
            </div>
          ))}
        </div>
      )}
      ...
      <div className="mx-4 mt-4 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-gray-50">
          ...
          {supv?.full_name && (
            <Row label="Supervisor" value={supv.full_name} />
          )}
